...
export default function EjecutivoPanel() {
  ...
  return (
    <div className="space-y-4">
      <h2 className="label-art text-neon-blue text-xl">Juicio ejecutivo (arts. 434-478 CPC)</h2>
        ...
        <div className="grid sm:grid-cols-2 gap-2">
          {TITULOS_EJECUTIVOS.map((t, i) => (
            <button key={t.n} onClick={() => setTitulo(i)} className={`p-3 border text-left text-xs ${titulo === i ? "border-neon-blue bg-neon-blue/10" : "border-ink-400"}`}>
              <div className="text-neon-cyan">N°{t.n}: {t.t}</div>
      ...
      <div className="terminal p-4">
        ...
        <p className="text-parchment/70 text-xs mt-3">
      ...
      <div className="terminal p-4">
        ...
        {oposicion && <p className="text-neon-cyan text-xs mt-2">Oposición recibida. Si se rechaza in limine (art. 466), el mandamiento basta de sentencia (art. 472).</p>}
      </div>
    </div>
  );
}
